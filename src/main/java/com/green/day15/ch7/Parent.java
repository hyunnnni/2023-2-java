...
public class Parent {//모든 객체는 오브젝트를 상속 받는다. 오브젝트는 최상위 객체 그래서 클래스명 뒤 extends Object가 생략되어있다
    ...
    public Parent(){
        ...
        //그래서 객체생성()에 빨간 줄이 뜬다거나 자식 클래스명에 전체 빨간 줄이 뜨면 기본생성자가 없다는 것이다.
        //그럼 부모 클래스에 기본생성자를 만들어주거나 자식 클래스에서 부모 클래스에 있는 다른 생성자와 타입을 맞춰 적어준다.
        //super 키워드는 생성자안에 생성자를 호출할 때 (this 쓸 때)한번만 호출해야한다
        //자식과 부모가 가진 오버로딩 메소드 중 부모의 메소드를 호출하고 싶을 때 super.을 쓰면 된다
        //this랑 같은 개념이라 사용법도 똑같다
    }
