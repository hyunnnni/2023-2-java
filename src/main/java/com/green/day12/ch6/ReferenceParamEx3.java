...
public class ReferenceParamEx3 {
    ...
    public static void sortArr(int[] arr) {//오름차순 오른쪽부터 큰 수가 채워진다
        int tmp = 0;
        for (int i = arr.length - 1; i > 0; i--) {//비교가 되야 하는 수
            for (int z = 0; z < i; z++) {//한 줄씩 검증할 때 되어야 하는 수( i가 점점 줄어든다 )
                //큰 자리 수가 오른쪽에 먼저 차례대로 정열되니깐 정열될 수록 검증할 필요가 없으니깐
                int nIdx = z + 1;//바로 다음 방을 비교하기 위한 변수
                if (arr[z] > arr[nIdx]) {//한 방씩 비교, z가 점점 증가하면서 다음 방으로 차례대로 비교가 된다
                    //앞 방의 크기가 뒷 방보다 크다면 두자리를 스와핑
                    tmp = arr[z];
                    arr[z] = arr[nIdx];
