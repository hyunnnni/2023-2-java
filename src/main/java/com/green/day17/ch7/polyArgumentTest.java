...
class Product{
    ...
    protected Product(int a){//같은 패키지 안이랑 다른 패키지 자손 클래스에 접근이 가능p.348
        price = a;
...
class Tv extends Product{
    ...
    //부모의 기본생성자를 호출한다. 그런데 부모클래스에 기본생성자가 없을 경우 ()에 에러가 생긴다
    //해결 방법 부모 클래스에 기본생성자를 넣거나 있는 다른 생성자를 이용한다 ()안에 파라미터값을 넣어준다
    //부모클래스에 기본생성자가 없이 자손클래스에 기본생성자를 적는 건 불가능하다 그걸 해결하는 방법이 위와 같은 이유
    //자손클래스가 객체생성을 해서 실행될 때 부모클래스도 같이 실행?되니깐 생성자가 있어야함,,,,
